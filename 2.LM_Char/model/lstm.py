...
class ChineseLSTM(nn.Module):
    def __init__(self, vocab_size, embedding_dim=128, hidden_dim=256, 
                 n_layers=2, dropout=0.3):
        super(ChineseLSTM, self).__init__()
...
class TextGenerator:
    ...
    def generate_text(self, start_text, length=100, temperature=1.0):
        """生成文本"""
        self.model.eval()
        ...
        with torch.no_grad():
            for _ in range(length):
                ...
                input_seq = torch.cat([input_seq[:, 1:], 
                                     torch.tensor([[predicted]], dtype=torch.long)], dim=1)
